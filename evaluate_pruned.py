...
evaluate_pruned.py
------------------
Evaluates a pruned + fine-tuned .pth model (from step3) on the test set.

Differences from evaluate.py:
  - Builds the pruned architecture from a --pruned_widths string instead of
    using fixed Mb_Tiny widths.
  - Otherwise identical: same metrics (per-class accuracy, mAP@0.5, confusion
    matrix), same NMS, same JSON GT loading.

Usage:
    python evaluate_pruned.py \
        --dataset_dir  /path/to/dataset \
        --model_path   models/pruning/finetuned_best.pth \
...
def build_pruned_backbone(widths):
    ...
    return nn.Sequential(
        ...
        conv_dw(w[4],  w[5],  1),
        conv_dw(w[5],  w[6],  1),
        conv_dw(w[6],  w[7],  1),   # tap 1, FIXED 64
        conv_dw(w[7],  w[8],  2),
        conv_dw(w[8],  w[9],  1),
        conv_dw(w[9],  w[10], 1),   # tap 2, FIXED 128
        conv_dw(w[10], w[11], 2),
        conv_dw(w[11], w[12], 1),   # tap 3, FIXED 256
    )


def build_pruned_model(widths, num_classes, device):
    """Build SSD model with pruned backbone + matching heads. is_test=True."""
    ...
    tap1_ch  = widths[7]   # 64
    tap2_ch  = widths[10]  # 128
    tap3_ch  = widths[12]  # 256
    extra_ch = 256

    extras = ModuleList([
        Sequential(
            Conv2d(tap3_ch, 64, kernel_size=1), ReLU(),
            SeperableConv2d(64, extra_ch, kernel_size=3, stride=2, padding=1), ReLU()
        )
    ])
    ...
    net = SSD(num_classes, backbone, [8, 11, 13],
...
# ── Main evaluation ───────────────────────────────────────────────────────────
def evaluate(dataset_dir, model_path, widths, device):
    ...
    print("\n[2/3] Loading pruned model ...")
    num_classes = len(CLASS_NAMES)
    net = build_pruned_model(widths, num_classes, str(device))
    net.load(model_path)
    net.to(device)
    net.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pruned_widths",  type=str, required=True,
    ...
    evaluate(args.dataset_dir, args.model_path, widths, args.device)
